...
思路：先统一除公因数25（如果有余数就补一，即使26你还要再分一次，用到Math.ceil函数，向上取整，并且记得（double）i / 25, 因为1/25 = 0向上取整还是0是不对的 ，但是（double）1 / 25 = 0.04向上取整是一），可以把常数减小一部分，降低一点时间复杂度，dp[n] [n]表示当前所给n毫升下所要求的概率

那么：dp[n] [n] = 0.25 * (dp[n - 4] [n] + dp[n - 3] [n - 1] + dp[n - 2] [n - 2] + dp[n - 1] [ n - 3]) 也就是一共四种取法，每种概率都是四分之一就可以呗。

![image-20221122172044809](https://cdn.jsdelivr.net/gh/Miyuki7/image-host/blog-imgimage-20221122172044809.png)

初始化：dp[0] [0] = 0.5，这时候**汤A和汤B** 同时分配完的概率 / 2，就是1 / 2 = 0.5。 

dp[0] [i] = 1.0，一定是A先分配完

dp[i] [0]  = 0, 两者都不满足，所以一定是0

**注意：数据量0 <= n <= 10的九次方，n平方的解法一定会超时，再结合题意，当n足够大时，这个概率一定是接近1的，所以可以求一下，当n = 4451时候概率就是0.99999了，这样就可以再最前面加上这个判断。**

``` java
//自下向上的动态规划
执行用时：
3 ms, 在所有 Java 提交中击败了76.28%的用户
内存消耗：
38.5 MB, 在所有 Java 提交中击败了85.37%的用户
...
public double soupServings(int n) {
    ...
    double dfs(int i, int j, double[][] dp) {
        if (i <=0 && j <= 0) {
            return 0.5;
        } else if (i <= 0) {
            return 1.0;
        ...
        dp[i][j] = 0.25 * (dfs(i - 4, j, dp) + dfs(i - 3, j - 1, dp) + dfs(i - 2, j - 2, dp) + dfs(i - 1, j - 3, dp));
        return dp[i][j];
    }
```
